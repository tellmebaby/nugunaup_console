...
function MinimumPriceInput({ bidId, acNo, minimumPrice, onSaved }: { bidId: number, acNo: number, minimumPrice: number | null, onSaved: (price: number|null) => void }) {
 ...
 const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/minimum-price/set`, {
        method: 'POST',
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/minimum-price/${acNo}`, {
        method: 'DELETE',
...
export default function NSAAppVehicleBid() {
  ...
  const selectWinner = async (winnerId: number, acNo: number) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/nsa-app-vehicle-bid/${acNo}/award`, {
        method: 'POST',
        headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' },
  ...
  const updateBidStatus = async (id: number, newStatus: string) => {
    try {
      ...
      const response = await fetch(`${API_BASE}/nsa-app-vehicle-bid/status`, {
        method: 'PUT',
        headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' },
        body: JSON.stringify({
          id: id,
        ...
        if (newStatus === '확인') {
          try {
            ...
            if (targetBid) {
              ...
              const actionTradeResponse = await fetch(`${API_BASE}/bid/action-trade`, {
                method: 'POST',
                headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' },
  ...
  const fetchVehicleBids = async (pageNum: number, limit: number = 10): Promise<ApiResponse> => {
    try {
      const response = await fetch(`${API_BASE}/nsa-app-vehicle-bid/list?page=${pageNum}&limit=${limit}`, {
        method: 'GET',
        headers: getAuthHeaders()
      });
